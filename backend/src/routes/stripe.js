...
const { assertPermanentEmail, isDisposableEmail } = require('../utils/disposableEmail');
...
router.post(
  ...
  try {
    ...
    const guestEmail = !userId ? (req.body.email || '').trim().toLowerCase() : undefined;
    ...
    if (!userId) {
      ...
      assertPermanentEmail(guestEmail);
    }
...
const fulfillCheckoutSession = async (checkoutSessionId) => {
  ...
  const checkoutEmail = (customer.email || session.metadata?.guestEmail || '').toLowerCase();
  ...
  try {
    order = await Order.create({
      ...
      guestEmail: !userId ? checkoutEmail : undefined,
      items: orderItems,
      shippingAddress,
      ...
      paymentStatus: 'paid',
      status: 'processing',
      adminNotes: `${isDisposableEmail(checkoutEmail) ? 'REVIEW: disposable email detected after Stripe Checkout. Confirm before fulfillment.\n' : ''}Stripe Checkout Session: ${session.id}`,
  ...
  const emailAddress = checkoutEmail;
  if (emailAddress) {
    try {
